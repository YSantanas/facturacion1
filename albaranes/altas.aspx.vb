...
Partial Class albaranes_altas
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                'Asigno Opcion Valorado/sin Valorar
                If tabla_empresa.Rows(0)("albaran_sin_valorar") = True Then
                ...
                If Request.QueryString("id_albaran") <> Nothing Then
                    ...
                    'Llamar al Albarán
                    consultar_factura(Request.QueryString("id_albaran"))
                ...
                If Request.QueryString("id_presupuesto") <> Nothing Then
                    ...
                    consultar_presupuesto(Request.QueryString("id_presupuesto"))
    ...
    Sub consultar_factura(ByVal n_factura As String)
        ...
        'Obtengo la Cabecera
        Dim tabla_cabecera As DataTable = funciones_globales.obtener_datos("SELECT * FROM " & tabla_empresa.Rows(0)("ruta_base_datos") & ".[dbo].[cabecera_albaranes] WHERE n_albaran='" & n_factura & "';")
        ...
        'Asigno
        If tabla_cabecera.Rows(0)("facturado") = True Then
            img_convertir_factura.Visible = False
            img_duplicar.Visible = False
            PL_detalles.Enabled = False
        Else
            img_convertir_factura.Visible = True
            img_duplicar.Visible = True
    ...
    Sub consultar_presupuesto(ByVal n_presupuesto As String)
        ...
        'Obtengo la Cabecera
        Dim tabla_cabecera As DataTable = funciones_globales.obtener_datos("SELECT * FROM " & tabla_empresa.Rows(0)("ruta_base_datos") & ".[dbo].[cabecera_presupuestos] WHERE n_presupuesto='" & n_presupuesto & "';")
        ...
        'Obtengo los Detalles
        Dim tabla_detalles As DataTable = funciones_globales.obtener_datos("SELECT fecha_creacion,hora_creacion,linea,cantidad,cod_articulo AS codigo,denominacion AS articulo,precio,dto_1 AS dto,porcentaje_impuesto AS impuesto,total,observaciones,imagen FROM " & tabla_empresa.Rows(0)("ruta_base_datos") & ".[dbo].[detalles_presupuestos] WHERE n_presupuesto='" & n_presupuesto & "' ORDER BY linea;")

        'Traer las imagenes
        For x = 0 To tabla_detalles.Rows.Count - 1

            Dim nombre As String = tabla_detalles.Rows(x)("imagen").ToString
            Dim ruta_origen As String = "D:/imagenes_usuarios_facturacion/" & tabla_empresa.Rows(0)("ruta_base_datos") & "/presupuesto/imagenes/"
            Dim ruta_destino As String = "D:\imagenes_usuarios_facturacion\temp\"

            'Copio la imagen
            If System.IO.File.Exists(ruta_origen & nombre) Then

                'Copio
                System.IO.File.Copy(ruta_origen & nombre, ruta_destino & nombre, True)
    ...
    Protected Sub GV_detalles_factura_DataBound(sender As Object, e As EventArgs) Handles GV_detalles_factura.DataBound
        ...
        Try
            ...
            Else
                ...
                'Si es un Albarán nuevo o una actualización
    ...
    Private Sub GV_detalles_factura_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GV_detalles_factura.RowCommand
        ...
        Try
            ...
            If (e.CommandName = "btn_add") Then
                ...
                'Asigno imagen
                If My.Computer.FileSystem.FileExists("D:/imagenes_usuarios_facturacion/" & tabla_empresa.Rows(0)("ruta_base_datos") & "/albaran/imagenes/" & GV_detalles_factura.DataKeys(index).Item("imagen").ToString) = True Then
                    img_observaciones.ImageUrl = "~/imagenes_usuarios_facturacion/imagenes_usuarios_facturacion/" & tabla_empresa.Rows(0)("ruta_base_datos") & "/albaran/imagenes/" & GV_detalles_factura.DataKeys(index).Item("imagen").ToString & "?" + numero_aleatorio.Next(100, 999999999).ToString()
                    img_observaciones.DataBind()
    ...
    Protected Sub btn_si_eliminar_Click(sender As Object, e As EventArgs) Handles btn_si_eliminar.Click
        ...
        Try
            ...
            If My.Computer.FileSystem.FileExists("D:/imagenes_usuarios_facturacion/" & tabla_empresa.Rows(0)("ruta_base_datos") & "/albaran/imagenes/" & GV_detalles_factura.DataKeys(txt_index.Text).Item("imagen").ToString) = True Then

                'Elimino
                System.IO.File.Delete("D:/imagenes_usuarios_facturacion/" & tabla_empresa.Rows(0)("ruta_base_datos") & "/albaran/imagenes/" & GV_detalles_factura.DataKeys(txt_index.Text).Item("imagen").ToString)
    ...
    Private Sub btn_observaciones_Click(sender As Object, e As EventArgs) Handles btn_observaciones.Click
        ...
        Try
            ...
            If FileUpload.HasFile Then
                ...
                'Asigno 
                Dim nombre As String = "Albaran_" & tabla_usuario.Rows(0)("Id").ToString & "_" & Date.Now.ToShortDateString.Replace("/", "") & "-" & Date.Now.Hour & Date.Now.Minute & Date.Now.Second & Date.Now.Millisecond & Path.GetExtension(FileUpload.FileName)
        ...
        Try
            ...
            For x = 0 To GV_detalles_factura.Rows.Count - 1
                ...
                If GV_detalles_factura.DataKeys(x).Item("imagen").ToString <> "" Then
                    ...
                    Dim ruta_imagen_origen As String = "D:\imagenes_usuarios_facturacion\" & tabla_empresa.Rows(0)("ruta_base_datos").ToString & "\albaran\imagenes\"
    ...
    Private Sub img_convertir_factura_Click(sender As Object, e As ImageClickEventArgs) Handles img_convertir_factura.Click
        ...
        Try
            ...
            'Asigno
            LT_mensaje_abonar.Text = "¿Está seguro de Convertir en Factura?"
        ...
        Catch ex As Exception
            'Registro Error
            funciones_globales.grabar_registro_error(tabla_empresa.Rows(0)("Id"), tabla_usuario.Rows(0)("Id"), Request.Url.Segments.Last(), "img_convertir_factura_Click", ex.Message)
            'Registro como bloque en local para el jquery
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "Notificacion_error", "Error('Error img_convertir_factura_Click: " & ex.Message.Replace("'", " ") & "');", True)
        End Try
    ...
    Private Sub btn_convertir_factura_confirmar_Click(sender As Object, e As EventArgs) Handles btn_convertir_factura_confirmar.Click
        ...
        Try
            ...
            'Registro como bloque en local para el jquery
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "abrir_ventana", "$('#ialtas___',window.parent.document).attr('src',$('#ialtas___',window.parent.document).attr('src'));window.parent.abrir_ventana_relacional('altas___','bi bi-columns-gap','1000','600','facturas/altas.aspx|id_albaran=" & txt_albaran.Text & "','4');", True)
    ...
    Protected Sub btn_grabar_Click(sender As Object, e As EventArgs) Handles btn_grabar.Click
        ...
        Try
            ...
            'Asigno
            LT_mensaje_confirmar.Text = "¿Está seguro de continuar y generar un Albarán?"
    ...
    Protected Sub btn_grabar_confirmar_Click(sender As Object, e As EventArgs) Handles btn_grabar_confirmar.Click
        ...
        Try
            ...
            Dim n_albaran As String = Nothing
            Dim n_presupuesto As String = txt_presupuesto.Text
            Dim origen As String = "Albaran"
            ...
            Select Case tabla_empresa.Rows(0)("tipo_autonumerico_asientos")
                Case "normal" : n_albaran = funciones_globales.numero_albaran(tabla_empresa.Rows(0)("ruta_base_datos"))
            End Select

            'Asigno
            txt_albaran.Text = n_albaran
            ...
            'Grabo la Cabecera del Albarán
            funciones_globales.grabar_albaran_(tabla_empresa.Rows(0)("ruta_base_datos"), n_albaran, txt_fecha.Text, cod_cliente, txt_cliente.Text, txt_nif.Text, txt_direccion.Text, txt_cp.Text, txt_poblacion.Text, txt_provincia.Text, GV_detalles_factura, GV_totales, origen, n_presupuesto)
            ...
            'Limpio el trazador
            funciones_globales.grabar_registro(tabla_empresa.Rows(0)("ruta_base_datos"), tabla_usuario.Rows(0)("Id"), tabla_usuario.Rows(0)("nombre") & " " & tabla_usuario.Rows(0)("primer_apellido") & " " & tabla_usuario.Rows(0)("segundo_apellido"), "Altas", "Creó el Albarán: " & txt_albaran.Text & ".(Tiempo: " & tspan.ToString("hh\:mm\:ss\:ff") & ").")
            ...
            'Registro como bloque en local para el jquery
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "Notificacion_ok", "ok('Albarán creado correctamente.');", True)
    ...
    Private Sub btn_actualizar_Click(sender As Object, e As EventArgs) Handles btn_actualizar.Click
        ...
        Try
            ...
            'Asigno
            LT_mensaje_confirmar_actualizar.Text = "¿Está seguro de continuar y actualizar el Albarán?"
    ...
    Private Sub btn_actualizar_confirmar_Click(sender As Object, e As EventArgs) Handles btn_actualizar_confirmar.Click
        ...
        Try
            ...
            funciones_globales.modificar_albaran_(tabla_empresa.Rows(0)("ruta_base_datos"), txt_albaran.Text, GV_detalles_factura, GV_totales)
            ...
            'Limpio el trazador
            funciones_globales.grabar_registro(tabla_empresa.Rows(0)("ruta_base_datos"), tabla_usuario.Rows(0)("Id"), tabla_usuario.Rows(0)("nombre") & " " & tabla_usuario.Rows(0)("primer_apellido") & " " & tabla_usuario.Rows(0)("segundo_apellido"), "Altas", "Modificó el Albarán: " & txt_albaran.Text & ".(Tiempo: " & tspan.ToString("hh\:mm\:ss\:ff") & ").")
            ...
            'Registro como bloque en local para el jquery
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "Notificacion_ok", "ok('Albarán modificado correctamente.');", True)
    ...
    Private Sub LB_imprimir_Click(sender As Object, e As EventArgs) Handles LB_imprimir.Click
        ...
        Try
            ...
            Dim nombre_fichero As String = crear_pdf_salida(tabla_empresa.Rows(0)("ruta_base_datos"), txt_albaran.Text)
    ...
    Function crear_pdf_salida(ByVal ruta_base_datos As String, ByVal n_albaran As String) As String
        ...
        Dim nombre As String = "Albaran_" & tabla_usuario.Rows(0)("Id") & "_" & Date.Now.Day & Date.Now.Month & Date.Now.Year & "-" & Date.Now.Hour & Date.Now.Minute & Date.Now.Second & ".pdf"
        ...
        Session("albaran") = n_albaran & "|" & ruta_base_datos
        ...
        'Obtengo
        Dim tabla_detalles_factura As DataTable = funciones_globales.obtener_datos("SELECT * FROM [" & ruta_base_datos & "].[dbo].detalles_albaranes WHERE n_albaran='" & n_albaran & "';")
        ...
        For x = 0 To tabla_detalles_factura.Rows.Count - 1
            ...
            Else
                ...
                'Sin valorar
                If chk_valorado.Checked = True Then
                    cell = New iTextSharp.text.pdf.PdfPCell(New iTextSharp.text.Phrase("", f))
                Else
                    cell = New iTextSharp.text.pdf.PdfPCell(New iTextSharp.text.Phrase(tabla_detalles_factura.Rows(x)("precio"), f))
                End If
            ...
            If tabla_detalles_factura.Rows(x)("observaciones").ToString <> "" Or tabla_detalles_factura.Rows(x)("imagen").ToString <> "" Then
                ...
                If Not IsDBNull(tabla_detalles_factura.Rows(x)("imagen")) And tabla_detalles_factura.Rows(x)("imagen").ToString <> "" Then
                    ...
                    Dim fs As FileStream = New FileStream("D:\imagenes_usuarios_facturacion\" & tabla_empresa.Rows(0)("ruta_base_datos") & "\albaran\imagenes\" & tabla_detalles_factura.Rows(x)("imagen"), FileMode.Open, FileAccess.Read, FileShare.Read)
                    ...
                    imagenlogo = iTextSharp.text.Image.GetInstance("D:\imagenes_usuarios_facturacion\" & tabla_empresa.Rows(0)("ruta_base_datos") & "\albaran\imagenes\" & tabla_detalles_factura.Rows(x)("imagen")) 'Direccion a la imagen que se hace referencia
        ...
        'Insertamos la imagen de fondo
        imagendemo = iTextSharp.text.Image.GetInstance("D:\imagenes_usuarios_facturacion\" & ruta_base_datos & "\albaran\pie_albaran.png") 'Dirreccion a la imagen que se hace referencia
        ...
        'Genero un codigo QR si esta activada la gestion documental
        If tabla_empresa.Rows(0)("albaran_qr") = True Then

            'Insertamos la imagen de fondo
            imagendemo = iTextSharp.text.Image.GetInstance(Server.MapPath("..") & "\temp\" & funciones_globales.generar_qr_albaran(tabla_empresa.Rows(0)("ruta_base_datos"), tabla_usuario.Rows(0)("Id"), n_albaran)) 'Direccion a la imagen que se hace referencia
            imagendemo.SetAbsolutePosition(funciones_globales.puntos(155), funciones_globales.puntos(215)) 'Posicion en el eje cartesiano
            imagendemo.ScaleAbsoluteWidth(funciones_globales.puntos(15)) 'Ancho de la imagen
            imagendemo.ScaleAbsoluteHeight(funciones_globales.puntos(15)) 'Altura de la imagen
            oDoc.Add(imagendemo) ' Agrega la imagen al documento

        End If

        'Obtengo
        Dim tabla_pie_factura As DataTable = funciones_globales.obtener_datos("SELECT * FROM [" & ruta_base_datos & "].[dbo].pie_albaranes WHERE n_albaran='" & n_albaran & "';")
        Dim total_factura As Decimal = 0
        ...
        'Sin valorar
        If chk_valorado.Checked = True Then
            cb.ShowTextAligned(iTextSharp.text.pdf.PdfContentByte.ALIGN_CENTER, "", funciones_globales.puntos(184.5), funciones_globales.puntos(30), 0)
        Else
    ...
    Private Sub btn_confirmar_email_Click(sender As Object, e As EventArgs) Handles btn_confirmar_email.Click
        ...
        Try
            ...
            correo.Subject = "Albarán Nº: " & txt_albaran.Text & "."
            ...
            "Te adjuntamos el Albarán solicitado.</span> " &
            "</td>" &
            "<td style='width:25%;'></td>" &
            "</tr>" &
            "<tr>" &
            ...
            'Limpio el trazador
            funciones_globales.grabar_registro(tabla_empresa.Rows(0)("ruta_base_datos"), tabla_usuario.Rows(0)("Id"), tabla_usuario.Rows(0)("nombre") & " " & tabla_usuario.Rows(0)("primer_apellido") & " " & tabla_usuario.Rows(0)("segundo_apellido"), "Altas", "Envió la Factura: " & txt_albaran.Text & " al E-mail: " & txt_destinatario.Text & ".(Tiempo: " & tspan.ToString("hh\:mm\:ss\:ff") & ").")
...
Public Class itsEvents
    ...
    Dim funciones_globales As New funciones_globales
    Dim sesion_albaran() As String = HttpContext.Current.Session("albaran").split("|")
    ...
    Public Overrides Sub OnStartPage(ByVal writer As iTextSharp.text.pdf.PdfWriter, ByVal document As iTextSharp.text.Document)
        ...
        'Insertamos la imagen de fondo
        imagendemo = iTextSharp.text.Image.GetInstance("D:\imagenes_usuarios_facturacion\" & sesion_albaran(1) & "\albaran\fondo_albaran.png") 'Direccion a la imagen que se hace referencia
        imagendemo.SetAbsolutePosition(0, 0) 'Posicion en el eje cartesiano
        imagendemo.ScaleAbsoluteWidth(funciones_globales.puntos(210)) 'Ancho de la imagen
        ...
        'Obtengo
        Dim tabla_cabecera_factura As DataTable = funciones_globales.obtener_datos("SELECT * FROM [" & sesion_albaran(1) & "].[dbo].cabecera_albaranes WHERE n_albaran='" & sesion_albaran(0) & "';")
        ...
        cb.SetFontAndSize(fuente, 8)
        cb.ShowTextAligned(iTextSharp.text.pdf.PdfContentByte.ALIGN_CENTER, tabla_cabecera_factura.Rows(0)("n_albaran").ToString.ToUpper, funciones_globales.puntos(23), funciones_globales.puntos(219), 0) 'alineación, texto a escribir y posición     
        cb.ShowTextAligned(iTextSharp.text.pdf.PdfContentByte.ALIGN_CENTER, tabla_cabecera_factura.Rows(0)("n_presupuesto").ToString.ToUpper, funciones_globales.puntos(51), funciones_globales.puntos(219), 0) 'alineación, texto a escribir y posición  
        cb.ShowTextAligned(iTextSharp.text.pdf.PdfContentByte.ALIGN_CENTER, Mid(tabla_cabecera_factura.Rows(0)("fecha").ToString.ToUpper, 1, 10), funciones_globales.puntos(75), funciones_globales.puntos(219), 0) 'alineación, texto a escribir y posición     
