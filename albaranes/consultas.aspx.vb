...
Partial Class albaranes_consultas
    ...
    Protected Sub btn_consultar_Click(sender As Object, e As EventArgs) Handles btn_consultar.Click
        ...
        Try
            ...
            'Limpio el trazador
            funciones_globales.grabar_registro(tabla_empresa.Rows(0)("ruta_base_datos"), tabla_usuario.Rows(0)("Id"), tabla_usuario.Rows(0)("nombre") & " " & tabla_usuario.Rows(0)("primer_apellido") & " " & tabla_usuario.Rows(0)("segundo_apellido"), "Consultas de Albaranes", "Solicitó listado.(Tiempo: " & tspan.ToString("hh\:mm\:ss\:ff") & ").")
    ...
    Sub cargar_GV()
        ...
        Try
            ...
            Using memConn As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString & ";Initial Catalog=" & tabla_empresa.Rows(0)("ruta_base_datos"))
                ...
                'Compongo la SELECT
                query = "SELECT cabecera_albaranes.n_albaran,cabecera_albaranes.fecha_creacion,cabecera_albaranes.hora_creacion,cabecera_albaranes.fecha,cabecera_albaranes.cod_cliente,cabecera_albaranes.cliente,cabecera_albaranes.nif,cabecera_albaranes.total " &
                        ",detalles_albaranes.linea ,detalles_albaranes.cod_articulo,detalles_albaranes.denominacion FROM cabecera_albaranes,detalles_albaranes "
                filtro = "WHERE cabecera_albaranes.n_albaran=detalles_albaranes.n_albaran AND facturado='False' "
                orden = "ORDER BY cabecera_albaranes.n_albaran,detalles_albaranes.linea;"
                ...
                If txt_n_albaran.Text <> "" Then

                    filtro += "AND n_albaran=@n_albaran "
                    memComando.Parameters.Add("@n_albaran", Data.SqlDbType.VarChar, 15).Value = txt_n_albaran.Text
    ...
    Private Sub gridview_consulta_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gridview_consulta.RowCommand
        ...
        Try
            ...
            If (e.CommandName = "ver_albaran") Then

                'Registro como bloque en local para el jquery
                ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "abrir_ventana", "$('#ialtas_____',window.parent.document).attr('src',$('#ialtas_____',window.parent.document).attr('src'));window.parent.abrir_ventana_relacional('altas_____','bi bi-columns-gap','1000','600','albaranes/altas.aspx|id_albaran=" & gridview_consulta.DataKeys(index).Item("n_albaran") & "','6');", True)
    ...
    Protected Sub img_exportar_excel_Click(sender As Object, e As EventArgs) Handles img_exportar_excel.Click
        ...
        Try
            ...
            'Crear Excel
            Dim nombre As String = funciones_globales.crear_excel("Consulta de albaranes", gridview_consulta, tabla_usuario.Rows(0)("Id"))
    ...
    Protected Sub img_exportar_txt_Click(sender As Object, e As EventArgs) Handles img_exportar_txt.Click
        ...
        Try
            ...
            'Crear Excel
            Dim nombre As String = funciones_globales.crear_txt("Consulta de albaranes", gridview_consulta, tabla_usuario.Rows(0)("Id"))
