...
Partial Class clientes_altas
    ...
    Protected Sub btn_consultar_Click(sender As Object, e As EventArgs) Handles btn_consultar.Click
        ...
        Try
            ...
            'Limpio el trazador
            funciones_globales.grabar_registro(tabla_empresa.Rows(0)("ruta_base_datos"), tabla_usuario.Rows(0)("Id"), tabla_usuario.Rows(0)("nombre") & " " & tabla_usuario.Rows(0)("primer_apellido") & " " & tabla_usuario.Rows(0)("segundo_apellido"), "Altas", "Solicitó listado del cliente: " & txt_cliente.Text & ".(Tiempo: " & tspan.ToString("hh\:mm\:ss\:ff") & ").")
    ...
    Sub cargar_GV()
        ...
        Try
            ...
            Using memConn As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString & ";Initial Catalog=" & tabla_empresa.Rows(0)("ruta_base_datos"))
                ...
                If txt_cliente.Text = "" Then
                    memComando.CommandText = "SELECT * FROM clientes ORDER BY Id;"
                Else
                    memComando.CommandText = "SELECT * FROM clientes WHERE Id = @id ORDER BY id;"
    ...
    Protected Sub img_exportar_excel_Click(sender As Object, e As EventArgs) Handles img_exportar_excel.Click
        ...
        Try
            ...
            'Crear Excel
            Dim nombre As String = funciones_globales.crear_excel("Alta de Clientes", gridview_consulta, tabla_usuario.Rows(0)("Id"))
    ...
    Protected Sub img_exportar_txt_Click(sender As Object, e As EventArgs) Handles img_exportar_txt.Click
        ...
        Try
            ...
            'Crear Excel
            Dim nombre As String = funciones_globales.crear_txt("Alta de Clientes", gridview_consulta, tabla_usuario.Rows(0)("Id"))
    ...
    Protected Sub img_nueva_Click(sender As Object, e As EventArgs) Handles img_nueva.Click
        ...
        Try
            ...
            'Asigno
            lbl_titulo.Text = "Agregar Cliente"
            btn_grabar.Visible = True
            btn_modificar.Visible = False
            Dim tabla_consulta As DataTable = funciones_globales.obtener_datos("SELECT TOP (1) id FROM " & tabla_empresa.Rows(0)("ruta_base_datos") & ".[dbo].[clientes] ORDER BY id DESC")
    ...
    Private Sub gridview_consulta_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gridview_consulta.RowCommand
        ...
        Try
            ...
            If (e.CommandName = "borrar") Then
                ...
                'Restricción si existen lineas de facturas con ese cliente
                Dim tabla As DataTable = funciones_globales.obtener_datos("SELECT TOP(1) * FROM [" & tabla_empresa.Rows(0)("ruta_base_datos") & "].[dbo].cabecera_facturas WHERE cod_cliente=" & gridview_consulta.DataKeys(index).Item("id").ToString().Replace(",", ".") & ";")
                If tabla.Rows.Count <> 0 Then
                    'Registro como bloque en local para el jquery
                    ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "Error_", "error('Existen Facturas emitidas a este cliente y no se puede eliminar.')", True)
                    Exit Sub
                End If

                'Restricción si existen sucursales no se puede borrar
                tabla = funciones_globales.obtener_datos("SELECT TOP(1) * FROM [" & tabla_empresa.Rows(0)("ruta_base_datos") & "].[dbo].[sucursales] WHERE id_cliente='" & gridview_consulta.DataKeys(index).Item("id").ToString() & "' AND id<>0;")
                If tabla.Rows.Count <> 0 Then
                    'Registro como bloque en local para el jquery
                    ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "Error_", "error('Existen Sucursales para el cliente: " & gridview_consulta.DataKeys(index).Item("nombre_fiscal").ToString() & ", debe eliminar primero dichas sucursales.')", True)
                    Exit Sub
                End If

                'Asignamos
                txt_index.Text = index
                LT_mensaje.Text = "¿Está seguro de eliminar al Cliente: " & gridview_consulta.DataKeys(index).Item("nombre_fiscal").ToString() & " (" & gridview_consulta.DataKeys(index).Item("nif").ToString() & ") ?"
            ...
            If (e.CommandName = "consultar") Then
                ...
                'Asigno
                lbl_titulo.Text = "Consultar Cliente"
                ...
                'Registro como bloque en local para el jquery
                ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "alerta_borrar", funciones_globales.modal_register("$('#txt_cliente_modal').attr('disabled',true);$('#txt_nombre_fiscal').attr('disabled',true);$('#CB_activo').attr('disabled',true);$('#txt_nombre_comercial').attr('disabled',true);$('#txt_nif').attr('disabled',true);$('#DDL_tipo_via').attr('disabled',true);$('#txt_domicilio').attr('disabled',true);$('#txt_numero').attr('disabled',true);$('#txt_escalera').attr('disabled',true);$('#txt_piso').attr('disabled',true);$('#txt_puerta').attr('disabled',true);$('#DDL_provincia').attr('disabled',true);$('#DDL_localidad').attr('disabled',true);$('#DDL_pais').attr('disabled',true);$('#txt_cp').attr('disabled',true);$('#Lb_contactos').attr('disabled',true);$('#Lb_sucursales').attr('disabled',true);$('#Lb_sucursales').attr('disabled',true);setTimeout(function () { $('#txt_nombre_fiscal').focus();$('#txt_nombre_fiscal').select();}, 100);$('#modal_agregar').modal('show');"), True)
    ...
    Protected Sub btn_eliminar_confirmar_Click(sender As Object, e As EventArgs) Handles btn_eliminar_confirmar.Click
        ...
        Try
            ...
            Using memConn As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString & ";Initial Catalog=" & tabla_empresa.Rows(0)("ruta_base_datos"))
                ...
                memComando.CommandText = "DELETE FROM clientes WHERE id=" & gridview_consulta.DataKeys(txt_index.Text).Item("id") & ";"
                memComando.Connection = memConn
                memComando.ExecuteNonQuery()

                memComando.CommandText = "DELETE FROM sucursales WHERE id_cliente=" & gridview_consulta.DataKeys(txt_index.Text).Item("id") & " AND id=0;"
                memComando.Connection = memConn
                memComando.ExecuteNonQuery()

                memComando.CommandText = "DELETE FROM contactos WHERE id_cliente=" & gridview_consulta.DataKeys(txt_index.Text).Item("id") & ";"
                memComando.ExecuteNonQuery()
    ...
    Protected Sub btn_grabar_Click(sender As Object, e As EventArgs) Handles btn_grabar.Click
        ...
        Try
            ...
            Dim tabla_consulta As DataTable = funciones_globales.obtener_datos("SELECT TOP (1) * FROM " & tabla_empresa.Rows(0)("ruta_base_datos") & ".[dbo].[clientes] WHERE id=" & txt_cliente_modal.Text & ";")
            If tabla_consulta.Rows.Count <> 0 Then
                'Registro como bloque en local para el jquery
                ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "Error_", funciones_globales.modal_register("setTimeout(function () { $('#txt_cliente_modal').focus();$('#txt_cliente_modal').select();}, 100);$('#modal_agregar').modal('show');error('El Código esta asignado al cliente: " & tabla_consulta.Rows(0)("nombre_fiscal") & ".')"), True)
                Exit Sub
            End If
            ...
            tabla_consulta = funciones_globales.obtener_datos("SELECT TOP (1) * FROM " & tabla_empresa.Rows(0)("ruta_base_datos") & ".[dbo].[clientes] WHERE nif='" & txt_nif.Text & "';")
            If tabla_consulta.Rows.Count <> 0 Then
                'Registro como bloque en local para el jquery
                ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "Error_", funciones_globales.modal_register("setTimeout(function () { $('#txt_nif').focus();$('#txt_nif').select();}, 100);$('#modal_agregar').modal('show');error('El NIF esta asignado al cliente: " & tabla_consulta.Rows(0)("nombre_fiscal") & ".')"), True)
                Exit Sub
            End If
            ...
            Using memConn As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString & ";Initial Catalog=" & tabla_empresa(0)("ruta_base_datos"))
                ...
                'Agrego
                memComando.CommandText = "INSERT INTO clientes (id,fecha_creacion,hora_creacion,activo,nombre_fiscal,nombre_comercial,nif,tipo_via,calle,numero,escalera,piso,puerta,cp,pais,provincia,localidad,observaciones) VALUES " &
            ...
            'Limpio el trazador
            funciones_globales.grabar_registro(tabla_empresa.Rows(0)("ruta_base_datos"), tabla_usuario.Rows(0)("Id"), tabla_usuario.Rows(0)("nombre") & " " & tabla_usuario.Rows(0)("primer_apellido") & " " & tabla_usuario.Rows(0)("segundo_apellido"), "Altas", "Creó el Cliente: " & txt_nombre_fiscal.Text & ".(Tiempo: " & tspan.ToString("hh\:mm\:ss\:ff") & ").")
            ...
            'Registro como bloque en local para el jquery
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "Notificacion_ok", "ok('Cliente creado correctamente.');", True)
    ...
    Protected Sub btn_modificar_Click(sender As Object, e As EventArgs) Handles btn_modificar.Click
        ...
        Try
            ...
            Dim tabla_consulta As DataTable = funciones_globales.obtener_datos("SELECT TOP (1) * FROM " & tabla_empresa.Rows(0)("ruta_base_datos") & ".[dbo].[clientes] WHERE nif='" & txt_nif.Text & "' AND Id<>" & gridview_consulta.DataKeys(txt_index.Text).Item("id") & ";")
            If tabla_consulta.Rows.Count <> 0 Then
                'Registro como bloque en local para el jquery
                ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "Error_", funciones_globales.modal_register("setTimeout(Function () { $('#txt_nif').focus();$('#txt_nif').select();}, 100);$('#modal_agregar').modal('show');error('El NIF esta asignado al cliente: " & tabla_consulta.Rows(0)("nombre_fiscal") & ".')"), True)
                Exit Sub
            ...
            Using memConn As SqlConnection = New SqlConnection(ConfigurationManager.ConnectionStrings("ConnectionString").ConnectionString & ";Initial Catalog=" & tabla_empresa(0)("ruta_base_datos"))
                ...
                'Actualizo
                memComando.CommandText = "UPDATE clientes SET " &
                "activo=@activo, " &
            ...
            'Limpio el trazador
            funciones_globales.grabar_registro(tabla_empresa.Rows(0)("ruta_base_datos"), tabla_usuario.Rows(0)("Id"), tabla_usuario.Rows(0)("nombre") & " " & tabla_usuario.Rows(0)("primer_apellido") & " " & tabla_usuario.Rows(0)("segundo_apellido"), "Altas", "Modificó el Cliente: " & txt_nombre_fiscal.Text & ".(Tiempo: " & tspan.ToString("hh\:mm\:ss\:ff") & ").")
            ...
            'Registro como bloque en local para el jquery
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType, "Notificacion_ok", "ok('Cliente modificado correctamente.');", True)
